...
- Agent 数据按所选范围同步。已存在的 Run 记录若为非终态，则返回冲突；范围选择或中断传输导致记录缺失时，继续扫描已有文件，允许后续同步补齐。其他读取或解析错误仍显式失败。
- Agent 跨设备续接须在 Full 同步完成后进行，并保留完整运行材料；默认核心历史不足以恢复。同步不提供跨设备运行锁或活动快照一致性。
